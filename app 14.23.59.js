...
function openCategory(catId) {
  ...
  openModal(`分类：${cat.name}`, () => {
    const wrap = document.createElement("div");
    wrap.innerHTML = `<textarea class="input" id="newCardInput" placeholder="输入字卡内容（可一次粘贴多行，每行一张）" style="height:120px;padding:8px;font-family:inherit;resize:vertical;"></textarea><div class="row" style="margin-top:8px;"><button class="btn" id="addCardBtn" style="width:100%">添加（按行拆分）</button></div><div id="cardList"></div>`;
    const list = wrap.querySelector("#cardList");
    function renderList() {
      list.innerHTML = "";
      if (!cat.cards.length) { list.innerHTML = `<div class="empty">还没有字卡</div>`; return; }
...
document.getElementById("momentsNewBtn").addEventListener("click", () => {
  openModal("发动态", () => {
    const wrap = document.createElement("div");
    wrap.innerHTML = `<div class="row"><textarea class="input" id="momentText" style="height:80px;padding:8px;font-family:inherit;resize:vertical;" placeholder="这一刻的想法…"></textarea></div><div class="row"><input type="file" id="momentImg" accept="image/*" hidden><button class="btn secondary" id="momentImgBtn" style="width:100%">添加图片</button></div><div id="momentImgPreview"></div><button class="btn" id="momentPostBtn" style="width:100%;margin-top:10px;">发表</button>`;
    let imgData = null;
    wrap.querySelector("#momentImgBtn").addEventListener("click", () => wrap.querySelector("#momentImg").click());
    wrap.querySelector("#momentImg").addEventListener("change", async () => {
      const f = wrap.querySelector("#momentImg").files[0];
    ...
    wrap.querySelector("#momentPostBtn").addEventListener("click", () => {
      ...
      const newMoment = {
        id: uid(), from: "me", text: text || "[图片]",
        image: imgData, time: fmtFull(nowBeijing()),
        likes: 0, liked: false, comments: []
      };
      state.moments.push(newMoment);
      saveMoments(); modal.classList.remove("open"); renderMoments();
